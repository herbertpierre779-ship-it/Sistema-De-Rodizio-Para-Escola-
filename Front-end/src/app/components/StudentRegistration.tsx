...
import type { ClassItem, RegistrationCaptureMode, SchoolYear, StudentItem } from "../types/api";
...
export default function StudentRegistration() {
  ...
  const activeStep = STEPS_3[Math.min(Math.max(activeIndex, 0), STEPS_3.length - 1)];
  ...
  const deleteSelectedStudent = async () => {
    ...
    try {
      await studentsApi.remove(token, selectedStudent.id);
      const sorted = (await studentsApi.list(token)).sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      setStudents(sorted);
      setSelectedStudentId(sorted[0]?.id ?? null);
    ...
    <div className="grid gap-6 xl:grid-cols-[1fr_0.95fr]">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200 lg:p-7">
        ...
        <form onSubmit={submit} className="mt-6 space-y-5">
            ...
            {isCameraOpen ? (
              ...
              <div className="mt-4 rounded-[1.5rem] border border-slate-200 bg-white p-4">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <button type="button" onClick={openCamera} className="inline-flex items-center gap-2 rounded-2xl bg-orange-500 px-4 py-3 text-sm font-semibold text-white"><Camera className="h-4 w-4" />{capturedCount === 0 ? "Abrir câmera" : isHundred ? `Continuar ciclo ${Math.min(CYCLES, Math.floor(capturedCount / CYCLE_SIZE) + 1)}` : "Capturar/refazer"}</button>
          ...
          <button type="submit" disabled={isSubmitting || !classId || !fullName.trim() || !done || normalizeCpf(cpf).length !== 11} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-orange-500 px-5 py-4 font-semibold text-white disabled:bg-slate-300"><Save className="h-4 w-4" />{isSubmitting ? "Salvando cadastro..." : "Salvar cadastro"}</button>
        </form>
      </section>
      ...
      <section className="space-y-5">
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200">
          ...
          <div className="mt-4 space-y-3">
            {isLoading ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">Carregando alunos...</div> : filteredRecentStudents.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">Nenhum aluno cadastrado ainda.</div> : filteredRecentStudents.slice(0, 6).map((student) => <button key={student.id} type="button" onClick={() => setSelectedStudentId(student.id)} className={`flex w-full items-center gap-4 rounded-2xl border p-4 text-left ${student.id === selectedStudent?.id ? "border-orange-300 bg-orange-50" : "border-slate-200 bg-slate-50"}`}>{student.photo_url ? <img src={student.photo_url} alt={student.full_name} className="h-12 w-12 rounded-2xl object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-200 text-slate-500"><UserRound className="h-5 w-5" /></div>}<div><p className="font-semibold text-slate-900">{student.full_name}</p><p className="text-sm text-slate-600">{student.class_name}</p></div></button>)}
          </div>
        </div>
